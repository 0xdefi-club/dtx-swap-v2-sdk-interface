...
function g(t) {
  ...
  return (g = function(t) {
    ...
    function n() {
      return k(t, arguments, T(this).constructor)
    }
    return (
...
var I = 'setPrototypeOf' in Object,
  O = (function(t) {
    function e() {
      ...
      return (
        ...
        I && Object.setPrototypeOf(w(n), (this instanceof e ? this.constructor : void 0).prototype),
        n
      )
    }
    return v(e, t), e
  ...
  _ = (function() {
    ...
    t.getAddress = function(t, e) {
      var n
      return P({
        factoryAddress: null != (n = s[t.chainId]) ? n : '0x2EA9051d5a48eA2350b26306f2b959D262cf67e1',
        tokenA: t,
        tokenB: e
      })
    }
    ...
    return (
      ...
      (e.getOutputAmount = function(e) {
        if (
          ...
          n.equal(this.reserve0.quotient, p) || n.equal(this.reserve1.quotient, p))
        )
          throw new O()
        var u = this.reserveOf(e.currency),
          i = this.reserveOf(e.currency.equals(this.token0) ? this.token1 : this.token0),
          s = n.multiply(e.quotient, m),
          a = n.multiply(s, i.quotient),
          c = n.add(n.multiply(u.quotient, h), s),
          l = r.CurrencyAmount.fromRawAmount(e.currency.equals(this.token0) ? this.token1 : this.token0, n.divide(a, c))
        if (n.equal(l.quotient, p)) throw new E()
        return [l, new t(u.add(e), i.subtract(l))]
      }),
      (e.getInputAmount = function(e) {
        if (
          (this.involvesToken(e.currency) || o(!1),
          n.equal(this.reserve0.quotient, p) ||
            n.equal(this.reserve1.quotient, p) ||
            n.greaterThanOrEqual(e.quotient, this.reserveOf(e.currency).quotient))
        )
          throw new O()
        var u = this.reserveOf(e.currency),
          i = this.reserveOf(e.currency.equals(this.token0) ? this.token1 : this.token0),
          s = n.multiply(n.multiply(i.quotient, e.quotient), h),
          a = n.multiply(n.subtract(u.quotient, e.quotient), m),
          c = r.CurrencyAmount.fromRawAmount(
            e.currency.equals(this.token0) ? this.token1 : this.token0,
            n.add(n.divide(s, a), l)
          )
        return [c, new t(i.add(c), u.subtract(e))]
      }),
      (e.getLiquidityMinted = function(t, e, u) {
        t.currency.equals(this.liquidityToken) || o(!1)
        var i,
          s = e.currency.sortsBefore(u.currency) ? [e, u] : [u, e]
        if (((s[0].currency.equals(this.token0) && s[1].currency.equals(this.token1)) || o(!1), n.equal(t.quotient, p)))
          i = n.subtract(r.sqrt(n.multiply(s[0].quotient, s[1].quotient)), c)
        else {
          var a = n.divide(n.multiply(s[0].quotient, t.quotient), this.reserve0.quotient),
            l = n.divide(n.multiply(s[1].quotient, t.quotient), this.reserve1.quotient)
          i = n.lessThanOrEqual(a, l) ? a : l
        }
        if (!n.greaterThan(i, p)) throw new E()
      ...
      (e.getLiquidityValue = function(t, e, u, i, s) {
        ...
        if (
          ...
          if (n.equal(c, p)) a = e
            ...
            var l = r.sqrt(n.multiply(this.reserve0.quotient, this.reserve1.quotient)),
              m = r.sqrt(c)
            if (n.greaterThan(l, m)) {
              var h = n.multiply(e.quotient, n.subtract(l, m)),
                d = n.add(n.multiply(l, f), m),
                y = n.divide(h, d)
  ...
  C = (function() {
    function t(t, e, n) {
      ...
      for (var i, s = [u], a = x(t.entries()); !(i = a()).done; ) {
        var c = i.value,
          p = c[1],
          l = s[c[0]]
      ...
      y(t, [
        {
          ...
          get: function() {
            ...
            for (var t, e = [], n = x(this.pairs.entries()); !(t = n()).done; ) {
              var o = t.value,
                u = o[1]
              e.push(
...
var S = (function() {
  function t(t, e, n) {
    ...
    if (n === r.TradeType.EXACT_INPUT) {
      ...
      console.log(e.currency.equals(t.output), 'output'),
        console.log(e.currency, 'amount.currency'),
        console.log(t.output, 'route.output'),
        e.currency.equals(t.output) || o(!1),
        (u[u.length - 1] = e.wrapped)
  ...
  return (
    (e.minimumAmountOut = function(t) {
      if ((t.lessThan(p) && o(!1), this.tradeType === r.TradeType.EXACT_OUTPUT)) return this.outputAmount
      var e = new r.Fraction(l)
        .add(t)
        .invert()
    ...
    (e.maximumAmountIn = function(t) {
      if ((t.lessThan(p) && o(!1), this.tradeType === r.TradeType.EXACT_INPUT)) return this.inputAmount
      var e = new r.Fraction(l).add(t).multiply(this.inputAmount.quotient).quotient
      return r.CurrencyAmount.fromRawAmount(this.inputAmount.currency, e)
    }),
    (t.bestTradeExactIn = function(e, n, u, i, s, a, c) {
      var l = void 0 === i ? {} : i,
        f = l.maxNumResults,
        m = void 0 === f ? 3 : f,
        h = l.maxHops,
      ...
      for (var y = a.wrapped, v = u.wrapped, T = 0; T < e.length; T++) {
        ...
        if (
          ...
          !A.reserve0.equalTo(p) &&
          !A.reserve1.equalTo(p)
        ) {
          var q = void 0
          try {
          ...
          if (q.currency.equals(v))
            r.sortedInsert(c, new t(new C([].concat(s, [A]), n.currency, u), n, r.TradeType.EXACT_INPUT), m, F)
          else if (d > 1 && e.length > 1) {
            var k = e.slice(0, T).concat(e.slice(T + 1, e.length))
            t.bestTradeExactIn(k, n, u, { maxNumResults: m, maxHops: d - 1 }, [].concat(s, [A]), q, c)
          }
        }
      }
      return c
    ...
    (t.bestTradeExactOut = function(e, n, u, i, s, a, c) {
      var l = void 0 === i ? {} : i,
        f = l.maxNumResults,
        m = void 0 === f ? 3 : f,
        h = l.maxHops,
      ...
      for (var y = a.wrapped, v = n.wrapped, T = 0; T < e.length; T++) {
        ...
        if (
          ...
          if (q.currency.equals(v))
            r.sortedInsert(c, new t(new C([A].concat(s), n, u.currency), u, r.TradeType.EXACT_OUTPUT), m, F)
          else if (d > 1 && e.length > 1) {
            var k = e.slice(0, T).concat(e.slice(T + 1, e.length))
            t.bestTradeExactOut(k, n, u, { maxNumResults: m, maxHops: d - 1 }, [A].concat(s), q, c)
          }
        }
      }
      return c
  ...
  return (
    (t.swapCallParameters = function(t, e) {
      ...
      var i,
        ...
        p = j(t.maximumAmountIn(e.allowedSlippage)),
        l = j(t.minimumAmountOut(e.allowedSlippage)),
...
;(exports.FACTORY_ADDRESS = '0x2EA9051d5a48eA2350b26306f2b959D262cf67e1'),
  ...
  (exports.InsufficientReservesError = O),
  (exports.MINIMUM_LIQUIDITY = c),
  (exports.Pair = _),
  (exports.Route = C),
  (exports.Router = U),
  (exports.Trade = S),
  (exports.computePairAddress = P),
  (exports.inputOutputComparator = R),
